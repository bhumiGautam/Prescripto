...
const doctorList = async (req,res)=>{
    try {
        const doctors = await doctorModel.find({}).select(['-passwors' , '-email'])
...
const doctorDashboard = async (req,res) =>{
    try {
        ...
        const dashData = {
            earning , appointments : appointments.length, patients : patients.length , latestAppointments : appointments.reverse().slice(0,5)
        }
...
// api to updatte doctor profile for doctor pannel
