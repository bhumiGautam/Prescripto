  ...
  return  docInfo && (
    <div>
      ...
      <div className='flex flex-col sm:flex-row gap-4'>
        <div>
          {
            docInfo?.image && <img className='bg-primary w-full sm:max-w-72 rounded-lg'  src={docInfo.image} alt="" />
      ...
      <div className='sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700'>
        ...
        <div className='flex items-center gap-3 w-full overflow-x-scroll mt-4'>
          {docSlots.length && docSlots[slotIndex].map((item,index)=>(
            <p onClick={()=> setSlotTime(item.time)} className = {`text-sm font-light shrink-0 px-5 py-2 rounded-full cursor-pointer ${item.time === slotTime ? 'bg-primary text-white' : 'text-gray-400 border border-r-gray-300'}`}key={index}>
